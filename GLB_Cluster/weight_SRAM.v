module weight_SRAM #(
	parameter WEIGHT_SIZE = 8, // weight precision (from DOTA dataset)

	parameter WEIGHT_SRAM_DEPTH = 256, // weight GLB depth 
	// for largest conv (filter size = 9) #weights/cluster = 16 * 12 = 192 -> 256
	...
	input		signed [WEIGHT_SIZE-1 : 0]			data_in,

	// router 0 ports
	input									data_out_ready_0,
	output	reg								data_out_valid_0,
	output	reg	signed [WEIGHT_SIZE-1 : 0]			data_out_0,
	// router 1 ports
	input									data_out_ready_1,
	output	reg								data_out_valid_1,
	output	reg	signed [WEIGHT_SIZE-1 : 0]			data_out_1,
	// rounter 2 ports
	input									data_out_ready_2,
	output	reg								data_out_valid_2,
	output	reg	signed [WEIGHT_SIZE-1 : 0]			data_out_2,
	
	// control signals
	output reg [WEIGHT_SRAM_ADDRESS_SIZE-1 : 0] write_address,
	input										write_en, // enables write
	input		[WEIGHT_SRAM_ADDRESS_SIZE-1 : 0]	write_addr, // initial write address
	output	reg									write_done, // flags the end of write operation

	// router 0 controls
	input										read_en_0, // enables read from port 0
	input		[WEIGHT_SRAM_ADDRESS_SIZE-1 : 0]	read_addr_0, // initial read address
	output	reg									read_done_0, // flags the end of read operation
	// router 1 controls
	input										read_en_1, // enables read from port 1
	input		[WEIGHT_SRAM_ADDRESS_SIZE-1 : 0]	read_addr_1, // initial read address
	output	reg									read_done_1, // flags the end of read operation
	// router 2 controls
	input										read_en_2, // enables read from port 2
	input		[WEIGHT_SRAM_ADDRESS_SIZE-1 : 0]	read_addr_2, // initial read address
	output	reg									read_done_2, // flags the end of read operation

	output reg [WEIGHT_SRAM_ADDRESS_SIZE-1 : 0] read_address_0,
	output reg [WEIGHT_SRAM_ADDRESS_SIZE-1 : 0] read_address_1,
	output reg [WEIGHT_SRAM_ADDRESS_SIZE-1 : 0] read_address_2
);
...
reg signed [WEIGHT_SIZE-1 : 0] mem [0 : WEIGHT_SRAM_DEPTH-1];
